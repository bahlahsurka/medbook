...
// placeholders — but the shape below (src/alt/caption/width/height) is
// exactly what a future replacement needs to fill in, so a not-yet-available
// image can be added the same way once it exists. Every entry needs real,
// concise alt text: these are genuine medical diagrams carrying real
// information, not decoration, so a screen reader visitor needs to get the
// same idea from the alt text that a sighted visitor gets from looking at
// the image. width/height are the real file dimensions — set as HTML
// attributes (not just CSS) wherever these render, so the browser reserves
// the correct aspect ratio before a lazy-loaded image finishes downloading,
// instead of the page jumping once it does (layout shift).

export const LANDING_IMAGES = {
  heartFailurePathway: {
    src: '/images/landing/heart-failure-pathogenesis.jpg',
    width: 1352, height: 784,
