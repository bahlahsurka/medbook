export default function Sidebar({ open, entries, activeSystem, setActiveSystem,
  ...
  return (
    <div style={{ width:open?240:0, minWidth:open?240:0, background:'#fff',
      ...
      <div style={{ padding:'16px 16px 12px', borderBottom:'1px solid #e5e7eb', flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:30, height:30, borderRadius:8, background:'#2563eb',
          ...
          return (
            <div key={sys.name} onClick={() => setActiveSystem(sys.name)}
              ...
              style={{ display:'flex', alignItems:'center', padding:'7px 16px',
                ...
                background:isActive?`${c}0d`:'transparent', transition:'all .1s' }}>
              <span style={{ fontSize:12.5, flex:1, color:isActive?c:'#374151',
                fontWeight:isActive?600:400 }}>{sys.name}</span>
              {cnt > 0 && (
                <span style={{ fontSize:10, background:isActive?`${c}18`:'#f3f4f6',
                  color:isActive?c:'#9ca3af', borderRadius:10,
      ...
      <div style={{ padding:'10px 12px 14px', borderTop:'1px solid #e5e7eb', flexShrink:0 }}>
        ...
        <Btn onClick={onManageSystems} full style={{ marginBottom:6 }}>⚙ Manage Systems</Btn>
...
function Btn({ onClick, children, danger, full }) {
  return (
    <button onClick={onClick} style={{ background:danger?'#fef2f2':'#f3f4f6',
      color:danger?'#dc2626':'#374151', border:`1px solid ${danger?'#fecaca':'#e5e7eb'}`,
