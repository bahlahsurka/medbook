...
export default function AddEntry({ activeSystem, color, userId, onSaved, onCancel, userSystems }) {
  ...
  // Apply highlight to selected text in textarea
  const applyHL = (c) => {
  ...
  // When notes text changes, preserve valid highlights
  ...
  const save = async () => {
    ...
    try {
      let urls = [];
      if (images.length > 0) {
        setSS('Uploading images…');
        urls = await Promise.all(images.map(uploadImg));
      }
      setSS('Saving entry…');
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
        ...
        <Field label="REVIEW NOTES">
          ...
          {hlMode && <div style={{ fontSize:11, color:'#9ca3af', marginBottom:6 }}>
            Select text in the box below, then tap a colour to highlight it.
          </div>}
          <textarea
            ref={taRef}
            value={notes}
        ...
        <Field label="SCREENSHOTS / IMAGES">
          <div style={{ display:'flex', gap:10, marginTop:8, flexWrap:'wrap' }}>
            ...
            <div onClick={()=>!saving&&galRef.current?.click()}
              style={{ flex:1, minWidth:120, border:'2px dashed #d1d5db',
                borderRadius:8, padding:'20px 14px', textAlign:'center',
          ...
          <div style={{fontSize:11,color:'#9ca3af',marginTop:8}}>
            ℹ️ Text and highlights are auto-saved as draft. Images need to be re-added if you switch apps.
          </div>
        </Field>
