...
function renderHL(text, hls) {
  if (!hls?.length) return <span style={{whiteSpace:'pre-wrap'}}>{text}</span>;
  const sorted = [...hls].sort((a,b)=>a.start-b.start);
  const parts = []; let cur = 0;
  sorted.forEach(h => {
    if (h.start > cur) parts.push({t:text.slice(cur,h.start),hl:null});
    parts.push({t:text.slice(h.start,h.end),hl:h.color});
    cur = h.end;
  });
  if (cur < text.length) parts.push({t:text.slice(cur),hl:null});
...
function Lightbox({ images, start, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.9)',
      ...
      <div style={{position:'absolute',top:16,right:20,color:'#fff',fontSize:26,cursor:'pointer'}}
        onClick={onClose}>✕</div>
      {images.length>1 && <div style={{position:'absolute',top:18,left:'50%',transform:'translateX(-50%)',
        color:'#fff',fontSize:13,background:'rgba(0,0,0,.4)',padding:'4px 12px',borderRadius:20}}>
        {idx+1} / {images.length}</div>}
      {images.length>1 && <>
        <div onClick={e=>{e.stopPropagation();prev();}} style={{position:'absolute',left:12,
          color:'#fff',fontSize:32,cursor:'pointer',padding:'8px 14px',
          background:'rgba(0,0,0,.3)',borderRadius:8,userSelect:'none'}}>‹</div>
        <div onClick={e=>{e.stopPropagation();next();}} style={{position:'absolute',right:12,
          color:'#fff',fontSize:32,cursor:'pointer',padding:'8px 14px',
          background:'rgba(0,0,0,.3)',borderRadius:8,userSelect:'none'}}>›</div>
      </>}
...
export default function DetailView({ entry, onBack, onDeleted, onUpdated, userId }) {
  ...
  const exportPDF = () => {
    ...
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8">
      ...
      .tag{display:inline-block;padding:2px 8px;border-radius:4px;font-size:11px;font-weight:600;margin-right:6px;background:#e5e7eb}
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const ok = await updateDB({
        ...
        difficulty:editDiff, images:[...editImgs,...newUrls], highlights:editHL
      });
      ...
      <div style={{display:'flex',flexDirection:'column',gap:16}}>
        ...
        <F label="DIFFICULTY">
          <div style={{display:'flex',gap:8,marginTop:8,flexWrap:'wrap'}}>
            {DIFFICULTY.map(d=>(
              <button key={d} onClick={()=>setED(d)} style={{padding:'7px 16px',borderRadius:6,
                border:`1px solid ${editDiff===d?DIFF_COLOR[d]:'#e5e7eb'}`,cursor:'pointer',
        ...
        <F label="REVIEW NOTES — select text then choose highlight colour">
          <div style={{display:'flex',gap:6,marginTop:8,marginBottom:6,flexWrap:'wrap',alignItems:'center'}}>
            <span style={{fontSize:11,color:'#9ca3af',fontWeight:600}}>HIGHLIGHT:</span>
            {HL_COLORS.map((c,i)=>(
              <div key={i} onClick={()=>{
                const ta=document.getElementById('edit-notes-ta');
                if(!ta)return;
                const s=ta.selectionStart,e=ta.selectionEnd;
                if(s===e)return;
                setEHL(p=>[...p,{start:s,end:e,color:c}]);
              }} style={{width:20,height:20,borderRadius:4,background:c.bg,
                cursor:'pointer',border:'1px solid #e5e7eb',flexShrink:0}} />
            ))}
            <button onClick={()=>setEHL([])} style={{fontSize:11,background:'#f3f4f6',
              border:'1px solid #e5e7eb',borderRadius:5,padding:'3px 10px',
              cursor:'pointer',color:'#6b7280',fontWeight:600,fontFamily:'Inter,sans-serif'}}>Clear</button>
          </div>
          <textarea id="edit-notes-ta" value={editNotes} onChange={e=>{setEN(e.target.value);setEHL([]);}}
        ...
        <div style={{display:'flex',gap:10}}>
          <button onClick={saveEdit} disabled={saving} style={{background:color,color:'#fff',
            border:'none',borderRadius:8,padding:'11px 24px',fontSize:14,fontWeight:600,
            cursor:saving?'not-allowed':'pointer',opacity:saving?.7:1,fontFamily:'Inter,sans-serif'}}>
            {saving?'Saving…':'✓ Save Changes'}
          </button>
          <button onClick={()=>{setEditing(false);setNI([]);setErr('');}} style={{
            background:'#f3f4f6',color:'#6b7280',border:'1px solid #e5e7eb',
            borderRadius:8,padding:'11px 20px',fontSize:14,cursor:'pointer',
            fontFamily:'Inter,sans-serif'}}>Cancel</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{maxWidth:680,margin:'0 auto',fontFamily:'Inter,sans-serif'}}>
      ...
      <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:10,
        ...
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          <Btn color="#16a34a" bg="#f0fdf4" border="#bbf7d0" onClick={markReviewed}>✓ Reviewed</Btn>
          <Btn color="#2563eb" bg="#eff6ff" border="#bfdbfe" onClick={()=>setEditing(true)}>✎ Edit</Btn>
          <Btn color={entry.pinned?'#d97706':'#374151'} bg={entry.pinned?'#fffbeb':'#f9fafb'}
            border={entry.pinned?'#fde68a':'#e5e7eb'} onClick={togglePin}>
            {entry.pinned?'📌 Unpin':'📌 Pin'}
          </Btn>
          <Btn color="#374151" bg="#f9fafb" border="#e5e7eb" onClick={exportPDF}>⬇ PDF</Btn>
          <Btn color="#dc2626" bg="#fef2f2" border="#fecaca"
            onClick={deleteEntry} disabled={deleting}>{deleting?'…':'Delete'}</Btn>
        </div>
      </div>
      ...
      {entry.notes && (
        <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:10,
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',
            ...
            <button onClick={()=>setHlMode(p=>!p)} style={{fontSize:11,
              background:hlMode?'#fef9c3':'#f3f4f6',
          ...
          {hlMode && (
            <div style={{display:'flex',gap:6,marginBottom:10,flexWrap:'wrap',alignItems:'center'}}>
              <span style={{fontSize:11,color:'#9ca3af',fontWeight:600}}>COLOUR:</span>
              {HL_COLORS.map((c,i)=>(
                <div key={i} onClick={()=>applyHL(c)}
                  style={{width:22,height:22,borderRadius:4,background:c.bg,
                    cursor:'pointer',border:'1px solid #e5e7eb',flexShrink:0}} />
              ))}
          ...
          <div ref={notesRef} style={{lineHeight:1.85,fontSize:14,color:'#1f2937',
            userSelect:hlMode?'text':'auto'}}>
            {renderHL(entry.notes, viewHL)}
          </div>
          {hlMode && <div style={{fontSize:11,color:'#9ca3af',marginTop:10}}>
...
function Tag({ label, color }) {
  return <span style={{fontSize:11,fontWeight:500,background:`${color}12`,color,
    borderRadius:4,padding:'2px 8px',border:`1px solid ${color}25`}}>{label}</span>;
}

function Btn({ onClick, children, color, bg, border, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{background:bg,border:`1px solid ${border}`,
