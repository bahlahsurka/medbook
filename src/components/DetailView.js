...
// ── Highlight toolbar (JSX lives here, not in highlights.js) ─────────────
      ...
      {parts.map((p, i) =>
        p.hl
          ? <mark key={i} style={{ background:p.hl.bg, color:p.hl.text,
              borderRadius:2, padding:'0 2px' }}>{p.t}</mark>
          : <span key={i}>{p.t}</span>
      )}
    </span>
  );
    ...
    <div
      ...
      {images.length > 1 && (
        <div style={{ position:'absolute', top:20, left:'50%', transform:'translateX(-50%)',
          color:'#fff', fontSize:13, background:'rgba(0,0,0,.5)',
      ...
      {images.length > 1 && (
        <button onClick={prev}
          style={{ position:'absolute', left:12, background:'rgba(255,255,255,.15)',
            border:'none', color:'#fff', fontSize:28, cursor:'pointer',
            width:44, height:44, borderRadius:'50%',
            display:'flex', alignItems:'center', justifyContent:'center' }}>‹</button>
      )}

      {/* Image — click anywhere on image does nothing, click outside closes */}
      <img
        src={images[idx]} alt=""
        style={{ maxWidth:'90vw', maxHeight:'88vh', borderRadius:8,
      ...
      {images.length > 1 && (
        <button onClick={next}
          style={{ position:'absolute', right:12, background:'rgba(255,255,255,.15)',
            border:'none', color:'#fff', fontSize:28, cursor:'pointer',
            width:44, height:44, borderRadius:'50%',
            display:'flex', alignItems:'center', justifyContent:'center' }}>›</button>
      )}
...
export default function DetailView({ entry, onBack, onDeleted, onUpdated, userId }) {
  ...
  const updateDB = async (fields) => {
    const { data, error } = await supabase.from('entries')
      .update(fields).eq('id', entry.id).select().single();
  ...
  // ── Highlight: edit mode (textarea selection) ───────────────────────────
  const applyEditHL = (c) => {
  ...
  // ── Highlight: view mode (DOM selection) ───────────────────────────────
  const applyViewHL = (c) => {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const ok = await updateDB({
        ...
        difficulty: editDiff,
        images:     allImgs,
        highlights: editHL,   // preserved through edit
      });
  ...
  const cancelEdit = () => {
    ...
    setEI(entry.images || []); setEHL(entry.highlights || []);
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
        ...
        <F label="REVIEW NOTES">
          <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:8, marginBottom:6, flexWrap:'wrap' }}>
            ...
            }}>🖊 {hlEditOn ? 'Highlighting on' : 'Highlight text'}</button>
            {editHL.length > 0 && (
              <span style={{ fontSize:11, color:'#9ca3af' }}>
                {editHL.length} highlight{editHL.length !== 1 ? 's' : ''}
              </span>
            )}
          </div>
          {hlEditOn && (
            <>
              <HLToolbar onApply={applyEditHL} onRemove={removeEditHL} />
              <div style={{ fontSize:11, color:'#9ca3af', marginBottom:6 }}>
                Select text in the box below, then tap a colour.
              </div>
            </>
          )}
  ...
  return (
    <div style={{ maxWidth:680, margin:'0 auto', fontFamily:'Inter,sans-serif' }}>
      ...
      {lb !== null && (
        <Lightbox images={entry.images} start={lb} onClose={() => setLb(null)} />
      )}
      ...
      <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:10,
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:8, alignItems:'center', marginBottom:16 }}>
          <span style={{ fontSize:11, fontWeight:500, background:`${dc}12`, color:dc,
            borderRadius:4, padding:'2px 8px', border:`1px solid ${dc}25` }}>
            {entry.difficulty}
          </span>
          ...
          {entry.review_count > 0 && (
            <span style={{ fontSize:12, color:'#16a34a', fontWeight:600 }}>
              ...
              {entry.last_reviewed && ` · Last: ${fmt(entry.last_reviewed)}`}
            </span>
          )}
        </div>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          <AB color="#16a34a" bg="#f0fdf4" border="#bbf7d0" onClick={markReviewed}>✓ Reviewed</AB>
          <AB color="#2563eb" bg="#eff6ff" border="#bfdbfe"
            onClick={() => { setEditing(true); setEHL(entry.highlights || []); }}>✎ Edit</AB>
          <AB color={entry.pinned ? '#d97706' : '#374151'}
            bg={entry.pinned ? '#fffbeb' : '#f9fafb'}
            border={entry.pinned ? '#fde68a' : '#e5e7eb'}
            onClick={togglePin}>{entry.pinned ? '📌 Unpin' : '📌 Pin'}</AB>
          <AB color="#374151" bg="#f9fafb" border="#e5e7eb" onClick={exportPDF}>⬇ PDF</AB>
          <AB color="#dc2626" bg="#fef2f2" border="#fecaca"
            onClick={deleteEntry} disabled={deleting}>{deleting ? '…' : 'Delete'}</AB>
        </div>
      </div>
      ...
      {entry.notes && (
        <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:10,
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
            ...
            <div style={{ fontSize:10, color:'#9ca3af', letterSpacing:.8,
          ...
          <div ref={notesRef}
            style={{ lineHeight:1.85, fontSize:14, color:'#1f2937',
      ...
      {/* Images — horizontal scroll, tap to expand */}
            ...
            {entry.images.map((url, i) => (
              <img key={i} src={url} alt=""
                ...
                style={{ height:180, width:'auto', maxWidth:'80vw', flexShrink:0,
                  borderRadius:8, border:'1px solid #e5e7eb', cursor:'zoom-in',
...
function AB({ onClick, children, color, bg, border, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      fontWeight:600, fontFamily:'Inter,sans-serif', opacity:disabled ? .6 : 1
    }}>{children}</button>
  );
}
